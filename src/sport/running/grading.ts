...
import type { AgeGradeEventData, AgeGradeTable } from './age-grade-table.js'
...
export class RunningAgeGrade {
  ...
  private static readonly table = ageGradeTable as unknown as AgeGradeTable;
  ...
  normalize(event: string, time: number): number {
    ...
    if (!eventData) {
      throw new Error(
        `Unknown running age-grade event "${event}" for gender "${this.gender}"`
      );
    }
