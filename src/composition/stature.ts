...
export class AmericanWhiteStatureEstimator implements FemurStatureEstimator {
...
export class AmericanBlackStatureEstimator implements FemurStatureEstimator {
  estimate({ gender, femurLength }: FemurStatureParams): number {
    if (gender === Gender.Female) {
 ...
 */
export function universalStature(height: number, age: number): number {
 ...
 */
export function strideLength(gender: Gender, height: number): number {
