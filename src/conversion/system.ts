  ...
  export namespace conversion {
    ...
    export class UnitConverter {
      ...
      val = function(): number {
        ...
        let target = unitTable[this.targetUnit];
        let current = unitTable[this.currentUnit];
      ...
      static addUnit = function(base: string, prefixed: string, multiplier: number): void {
    ...
    let siPrefixes: string[] = ['Y', 'Z', 'E', 'P', 'T', 'G', 'M', 'k', 'h', 'da', '', 'd', 'c', 'm', 'u', 'n', 'p', 'f', 'a', 'z', 'y'];
    let siFactors: number[] = [24, 21, 18, 15, 12, 9, 6, 3, 2, 1, 0, -1, -2, -3, -6, -9, -12, -15, -18, -21, -24];
    let siUnits:string[] = ['g', 'b', 'L', 'm', 'A', 'cd', 'mol', 'K', 'are', 'Pa', 'J', 'W',];

    for (var j = 0; j < siUnits.length; j++) {
      let base: string = siUnits[j];
