...
abstract class PerformanceModel {
   ...
   */
  time(_d2: number): number { return 0}
   ...
   */
  distance(_t2: number): number { return 0}
...
export class VV implements TimeEstimatable {
   ...
   */
  time(mileage: number, d2=42195.0): number {
  ...
  time2(mileage: number, d2: number, t2: number, distance=42195.0): number {
    const adj_timer_r1: number = this.adj_timer(this.d1, this.t1);
    const adj_timer_r2: number = this.adj_timer(d2, t2);
    const k_r2_r1: number = Math.log(adj_timer_r2/adj_timer_r1)/Math.log(d2/this.d1);
    const k_marathon: number = 1.4510756+(-0.23797948*k_r2_r1)+(-0.01410023*(mileage/10) );
    const seconds: number = (adj_timer_r2*Math.pow(distance/d2, k_marathon) );
    return seconds;
  }
}